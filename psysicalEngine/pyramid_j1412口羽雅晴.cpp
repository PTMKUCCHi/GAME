...
void MakeWalls()
{
	...
	float restitution = 1.0;//反発力
	float friction = 0.0;//摩擦力
...
void MakeObjects()
{
	...
	//body = CreateBody(x, y, w, h, angle, isDynamic, isBall, density, restitution, friction);
	//}
	...
	for(i=0;i<10;i++){
		...
		for(j=0;j<10;j++){
			...
			body = CreateBody(CENTER_X, MAX_Y-100.0, w, h, angle, isDynamic, isBall, density, restitution, friction);
