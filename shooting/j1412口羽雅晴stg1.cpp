...
#define LEVEL_UP_SCORE 3000 //敵1機で100点、1000なら10機でレベルアップ
...
void LoadData()
{
	...
	Title_img=LoadGraph("title.PNG");
	Hplay_img=LoadGraph("H_Play.JPG");
...
void ActionPlayer()
{
	...
	{
		//敵を10倒すごとに+1方向(最大6方向)
		way = his_score > LEVEL_UP_SCORE * 5 ? way : his_score / LEVEL_UP_SCORE + 1;
	...
	//ショットを撃つ
	else if( CheckHitKey(KEY_INPUT_C)  && t % (fire+20) == 0 )
	{
		for(i=0;i<360;i+=30){
			MakeShot(speed, OMEGA(i),power,range);
		}
	}
		...
		for(j = 0; j < MAX_PLAYER_SHOT; j++)
		{
			...
			if( hypot (x,y) < enemy[i].range + shot[j].range )
			{
				...
				if(enemy[i].hp < 0)
				{
					...
					score += 100;//スコア加算
					his_score+=100;
					if( his_score % LEVEL_UP_SCORE == 0 && his_score > 0 && his_score < LEVEL_UP_SCORE * 6)
...
void MakeEnemy()
{
	...
	//ザコ敵 下側ランダム方向に直進(大型が出現した後はしばらく出ない)
	if( t % born1 == 0 && t % (born2 + 2500) < born2)
	{
		...
		enemy[i].hp = score/LEVEL_UP_SCORE*3;

		if(score<LEVEL_UP_SCORE){
			enemy[i].hp = score/LEVEL_UP_SCORE+1;
		}
...
void ActionEnemy()
{
	...
	while(enemy[i].action!=BOSS){
		i++;
	}

	const double angle2 = TargetAnglePlayer(enemy[i].x,enemy[i].y);//自機を追いかける
	...
	for(i = 0; i < MAX_ENEMY; i++){
		...
		case BOSS://何もしない
			...
			enemy[i].x = CENTER_X+rBOSS*cos(OMEGA(t));
			enemy[i].y = CENTER_Y/4;//CENTER_Y+rBOSS*sin(OMEGA(t));

			break;

		case STRAIGHT://決まった方向に直進

			enemy[i].x += enemy[i].speed * cos( OMEGA(t) );
		...
		case BOSS:
			...
			if(enemy[i].hp>=950){
				...
				if( t % fire*2 == 0 ){
					for(j=0;j<360;j+=30){
						MakeBullet(enemy[i].x, enemy[i].y, speed*1, OMEGA(j), range,bullet_img1);
					}
			...
			if(enemy[i].hp>900&&enemy[i].hp<=950){
				if( t % fire == 0 ){
					MakeWayBullet(enemy[i].x, enemy[i].y, speed*2, way*2, OMEGA(120), angle2, range,bullet_img1);
				}
			}

			if(enemy[i].hp>0&&enemy[i].hp<=900){

				if(t%(fire/2)==0){
					//MakeBullet(enemy.x, enemy.y, speed*4, angle, range);
					MakeWayBullet(enemy[i].x, enemy[i].y, speed*2, way*2, OMEGA(360), angle, range,bullet_img1);
				}
			}
			break;
